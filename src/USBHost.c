...
// Derived from atc1441/CH559sdccUSBHost (GPLv3); modified for this project
// (mouse decode + SPI output hooks in getHIDDeviceReport/pollHIDdevice).
...
/* UART debug output; `make RELEASE=1` compiles it out (see main.c / Makefile). */
#ifdef DEBUG_UART
...
struct
{
	...
	unsigned long type;
	MouseLayout layout; /* parsed mouse field positions (layout.x.valid if mouse) */
	unsigned char pollInterval; /* endpoint bInterval in ms (>=1); 0 until enumerated */
	unsigned char lastPoll;     /* timerMs() at the last IN poll, for pacing */
...
void resetHubDevices(unsigned char hubindex)
{
	...
	HIDdevice[hiddevice].layout.x.valid = 0;
	}
	}
}
...
#ifdef DEBUG_RAW
	 __xdata unsigned char di;
#endif
	for (hiddevice = 0; hiddevice < MAX_HID_DEVICES; hiddevice++)
	{
		if(HIDdevice[hiddevice].connected){
		/* Pace to the endpoint's bInterval: skip until that many ms elapsed
		 * since the last poll. Avoids flooding the bus with NAKed IN tokens
		 * between actual reports. Unsigned 8-bit subtraction wraps cleanly. */
		unsigned char now = timerMs();
		if ((unsigned char)(now - HIDdevice[hiddevice].lastPoll) < HIDdevice[hiddevice].pollInterval)
			continue;
		HIDdevice[hiddevice].lastPoll = now;
		...
		if ( s == ERR_SUCCESS )
   		{
			...
			if ( len )
			{
				if (HIDdevice[hiddevice].layout.x.valid)
				{
					MouseState ms;
					if (hid_extract_mouse(&HIDdevice[hiddevice].layout, RxBuffer, len, &ms))
					{
						spiUpdateMouse(&ms);
#ifdef DEBUG_RAW
						/* raw interrupt-IN report bytes (debug build, make RAW=1) */
						printf("RAW len=%d:", (int)len);
						for (di = 0; di < len; di++)
							printf(" %02X", (int)RxBuffer[di]);
						printf("  ");
#endif
						DBG("M b=%04X dx=%d dy=%d w=%d hw=%d\n",
						    (int)ms.buttons, (int)ms.dx, (int)ms.dy,
						    (int)ms.wheel, (int)ms.hwheel);
					}
				}
			}
		}
...
unsigned char getHIDDeviceReport(unsigned char CurrentDevive)
{
	...
	/* Extract mouse field bit positions for fast decoding during polling. */
	hid_parse_mouse(receiveDataBuffer, len, &HIDdevice[CurrentDevive].layout);
	if (HIDdevice[CurrentDevive].layout.x.valid)
		DBG("Mouse detected: dev=%d reportId=%d\n",
		    (int)CurrentDevive, (int)HIDdevice[CurrentDevive].layout.mouseReportId);
	return (ERR_SUCCESS);
}
								...
								case USB_DESCR_TYP_ENDP:
									...
									if(currentInterface->bInterfaceClass == USB_DEV_CLASS_HID)
									{
										...
										if(d->bEndpointAddress & 0x80){
											...
											/* Pace polling to the endpoint's bInterval (ms);
											 * clamp 0 to 1 ms so a bad descriptor still polls. */
											HIDdevice[hiddevice].pollInterval = d->bInterval ? d->bInterval : 1;
											HIDdevice[hiddevice].lastPoll = timerMs();
